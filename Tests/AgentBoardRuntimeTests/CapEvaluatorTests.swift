...
final class CapEvaluatorTests: XCTestCase {
    ...
    func testAWaitingSessionIsNotIdleButIsStillSubjectToTheOtherCaps() {
        ...
        let now = last.addingTimeInterval(61)
        XCTAssertEqual(
            CapEvaluator.evaluate(totals: .zero, startedAt: start, lastActivity: last, now: now, limits: limits),
        ...
        for state in [SessionState.waitingOnLock, .blocked, .setup] {
            XCTAssertNil(
                CapEvaluator.evaluate(
                    totals: .zero, startedAt: start, lastActivity: last, now: now, limits: limits, state: state
                ),
        ...
        XCTAssertEqual(
            CapEvaluator.evaluate(
                totals: .zero, startedAt: start, lastActivity: last, now: late, limits: limits,
                state: .waitingOnLock
            ),
        ...
        XCTAssertEqual(
            CapEvaluator.evaluate(
                ...
                startedAt: start, lastActivity: last, now: now, limits: limits, state: .waitingOnLock
            ),
