...
final class MessagePTYIsolationTests: XCTestCase {
    func testTheOnlyPTYWriteInTheProjectIsTheReportNotice() throws {
        ...
        XCTAssertEqual(writers["OrchestratorConsole.swift"]?.count, 1)
        XCTAssertTrue(
            writers["OrchestratorConsole.swift"]?.first?.contains("reports pending. Call list_reports.") == true,
            "the one PTY write is no longer the fixed report notice"
        )
    }
