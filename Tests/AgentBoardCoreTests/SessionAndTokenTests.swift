...
final class SessionStoreTests: XCTestCase {
    func testActiveFiltersByStateAndProject() throws {
        ...
        XCTAssertEqual(Set(active.map(\.state)), [.setup, .starting, .running, .idle, .blocked, .waitingOnLock])
    ...
    func testSessionStateIsActive() {
        XCTAssertEqual(
            SessionState.allCases.filter(\.isActive),
            [.setup, .starting, .running, .idle, .blocked, .waitingOnLock]
        )
    }
}
